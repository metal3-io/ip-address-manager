...
// Tests upgrades from older released versions => current.
var _ = Describe("When testing IPAM provider upgrade", Label("ipam", "upgrade"), func() {
	...
	AfterEach(func() {
		if upgradeClusterProxy != nil {
			cleanupNamespace(ctx, upgradeClusterProxy, namespace, artifactFolder, clusterctlConfigPath)
		}
		if skipCleanup {
			return
		}
	...
	runUpgradeTest := func(clusterName, oldIPAMVersion string) {
		...
		Expect(string(newAddr.Spec.Address)).ToNot(Equal(preUpgradeIP), "New allocation should get a different IP than the Metal3 allocation")
		Expect(string(newAddr.Spec.Address)).ToNot(Equal(preUpgradeCAPIIP), "New allocation should get a different IP than the CAPI allocation")

		Logf("Upgrade test passed: pre-upgrade IPs preserved, new allocations working")
	}

	// Test upgrade from an older release (v1.12) to current.
	...
	// Test upgrade from an older release (v1.13) to current.
