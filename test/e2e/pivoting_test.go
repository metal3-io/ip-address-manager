...
var _ = Describe("When testing clusterctl move of IPAM resources", Label("ipam", "pivot"), func() {
	...
	AfterEach(func() {
		// cleanupNamespace always dumps resources to artifacts first, then honors
		// skipCleanup for the namespace deletion.
		if targetClusterProxy != nil {
			cleanupNamespace(ctx, targetClusterProxy, namespace, artifactFolder, clusterctlConfigPath)
		}
	...
	It("Should move IPPool, IPClaim and IPAddressClaim resources to a target cluster", func() {
		...
		By("Verifying ownerReferences on the target reference the moved objects by their NEW UIDs")
		// clusterctl move recreates objects on the target with fresh UIDs. Verifying
		// ownerReferences point at the target-cluster UIDs (not stale source UIDs) is the
		// real check that ownership survived the move correctly.
		expectOwnedByUID(movedAddr1.OwnerReferences, "IPAddressClaim", movedClaim1.Name, movedClaim1.UID)
		expectOwnedByUID(movedAddr2.OwnerReferences, "IPAddressClaim", movedClaim2.Name, movedClaim2.UID)
		expectOwnedByUID(movedM3Addr.OwnerReferences, "IPClaim", movedM3Claim.Name, movedM3Claim.UID)
	...
	It("Should move an IPPool with preallocations to a target cluster", func() {
		...
		Expect(preMoveClaim1Addr.Spec.Address).To(Equal(string(prealloc1)), "claim1 should be assigned its configured preallocated IP before move")
		...
		Expect(preMoveClaim2Addr.Spec.Address).To(Equal(string(prealloc2)), "claim2 should be assigned its configured preallocated IP before move")
...
// expectOwnedByUID asserts that ownerRefs contains an ownerReference with the given
// kind, name and UID. After a clusterctl move, objects are recreated on the target
// with fresh UIDs, so this confirms ownerReferences were rewritten to the target's
// UIDs rather than left pointing at stale source-cluster UIDs.
func expectOwnedByUID(ownerRefs []metav1.OwnerReference, kind, name string, uid types.UID) {
	Expect(ownerRefs).To(ContainElement(SatisfyAll(
		HaveField("Kind", Equal(kind)),
		HaveField("Name", Equal(name)),
		HaveField("UID", Equal(uid)),
	)), "expected an ownerReference to %s/%s with target UID %s, got %+v", kind, name, uid, ownerRefs)
}
