...
func (m *IPPoolManager) updateAddressCounts(inUse int) error {
	...
	if m.IPPool.Status.CapacityCapped ||
		...
		m.IPPool.Status.CapacityCapped = false
...
// setCapacityCapped records that the combined pool capacity is too large to
// represent exactly: Capacity is capped at the int64 maximum, AvailableIPCount
// is left unknown (a huge pool is not actually empty), and CapacityCapped is
// raised.
func (m *IPPoolManager) setCapacityCapped() {
	...
	m.IPPool.Status.CapacityCapped = true
