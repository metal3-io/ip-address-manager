...
var _ = Describe("IPPool manager", func() {
	...
	DescribeTable("Test Capacity and AvailableIPCount computation",
		...
		Entry("Two large pools overflow int64: counts cleared", testCaseUpdateAddressCounts{
			pools: []ipamv1.Pool{
			...
			inUse:       0,
			expectError: true,
			// expectedCapacity / expectedAvailableIPCount left nil (cleared)
		}),
		Entry("Single huge IPv6 pool cannot be sized: counts cleared", testCaseUpdateAddressCounts{
			pools: []ipamv1.Pool{
