...
var _ = Describe("IPPool manager", func() {
	...
	DescribeTable("Test Capacity and AvailableIPCount computation",
		func(tc testCaseUpdateAddressCounts) {
			...
			Expect(ipPool.Status.CapacityCapped).To(Equal(tc.expectedCapacityCapped))
		},
		...
		Entry("Two large pools overflow int64: capacity capped", testCaseUpdateAddressCounts{
			pools: []ipamv1.Pool{
			...
			expectedCapacityCapped:   true,
		}),
		Entry("Single huge IPv6 pool too large to count: capacity capped", testCaseUpdateAddressCounts{
			pools: []ipamv1.Pool{
