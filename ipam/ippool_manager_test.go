...
var _ = Describe("IPPool manager", func() {
	...
	type testCaseK8sCreateAddresses struct {
		ipPool              *ipamv1.IPPool
	...
	DescribeTable("Test K8sCreateAddresses",
		func(tc testCaseK8sCreateAddresses) {
			objects := []client.Object{}
			for _, address := range tc.ipAddresses {
				objects = append(objects, address)
			}
			...
			allocatedMap, err := ipPoolMgr.K8sCreateAddress(context.TODO(), tc.ipAddressClaim,
				tc.addresses,
			)
			if tc.expectRequeue || tc.expectError {
				Expect(err).To(HaveOccurred())
		...
		Entry("Already exists", testCaseK8sCreateAddresses{
			ipPool: &ipamv1.IPPool{
				ObjectMeta: ipPoolMeta,
				Status: ipamv1.IPPoolStatus{
					Allocations: map[string]ipamv1.IPAddressStr{
		...
		Entry("Not allocated yet, pre-allocated", testCaseK8sCreateAddresses{
			ipPool: &ipamv1.IPPool{
				ObjectMeta: ipPoolMeta,
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
		...
		Entry("Not allocated yet", testCaseK8sCreateAddresses{
			ipPool: &ipamv1.IPPool{
				ObjectMeta: ipPoolMeta,
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
		...
		Entry("Not allocated yet, conflict", testCaseK8sCreateAddresses{
			ipPool: &ipamv1.IPPool{
				ObjectMeta: ipPoolMeta,
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
		...
		Entry("Not allocated yet, exhausted pool", testCaseK8sCreateAddresses{
			ipPool: &ipamv1.IPPool{
				ObjectMeta: ipPoolMeta,
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
	...
	type testK8sCaseAllocateAddress struct {
		ipPool          *ipamv1.IPPool
		addresses       map[ipamv1.IPAddressStr]string
		ipAddressClaim  *capipamv1.IPAddressClaim
		expectedAddress ipamv1.IPAddressStr
	...
	DescribeTable("Test K8sAllocateAddress",
		func(tc testK8sCaseAllocateAddress) {
			ipPoolMgr, err := NewIPPoolManager(nil, tc.ipPool,
				logr.Discard(),
			)
			Expect(err).NotTo(HaveOccurred())
			allocatedAddress, prefix, gateway, err := ipPoolMgr.K8sAllocateAddress(
		...
		Entry("Empty pools", testK8sCaseAllocateAddress{
			ipPool: &ipamv1.IPPool{
				Spec: ipamv1.IPPoolSpec{},
			},
			ipAddressClaim: &capipamv1.IPAddressClaim{
		...
		Entry("One pool, pre-allocated", testK8sCaseAllocateAddress{
			ipPool: &ipamv1.IPPool{
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
						{
		...
		Entry("One pool, pre-allocated, with overrides", testK8sCaseAllocateAddress{
			ipPool: &ipamv1.IPPool{
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
						{
		...
		Entry("One pool, pre-allocated, out of bonds", testK8sCaseAllocateAddress{
			ipPool: &ipamv1.IPPool{
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
						{
		...
		Entry("One pool, with start and existing address", testK8sCaseAllocateAddress{
			ipPool: &ipamv1.IPPool{
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
						{
		...
		Entry("One pool, with subnet and override prefix", testK8sCaseAllocateAddress{
			ipPool: &ipamv1.IPPool{
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
						{
		...
		Entry("two pools, with subnet and override prefix in first", testK8sCaseAllocateAddress{
			ipPool: &ipamv1.IPPool{
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
						{
		...
		Entry("two pools, with subnet and override prefix", testK8sCaseAllocateAddress{
			ipPool: &ipamv1.IPPool{
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
						{
		...
		Entry("Exhausted pools start", testK8sCaseAllocateAddress{
			ipPool: &ipamv1.IPPool{
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
						{
		...
		Entry("Exhausted pools subnet", testK8sCaseAllocateAddress{
			ipPool: &ipamv1.IPPool{
				Spec: ipamv1.IPPoolSpec{
					Pools: []ipamv1.Pool{
						{
	...
	type testCaseK8sDeleteAddresses struct {
		ipPool              *ipamv1.IPPool
		ipAddressClaim      *capipamv1.IPAddressClaim
		capiAddresses       []*capipamv1.IPAddress
		addresses           map[ipamv1.IPAddressStr]string
	...
	DescribeTable("Test K8sDeleteAddresses",
		func(tc testCaseK8sDeleteAddresses) {
			objects := []client.Object{}
			for _, address := range tc.capiAddresses {
				objects = append(objects, address)
			}
			...
			allocatedMap, err := ipPoolMgr.K8sDeleteAddress(context.TODO(), tc.ipAddressClaim, tc.addresses)
			if tc.expectError {
				Expect(err).To(HaveOccurred())
			} else {
				Expect(err).NotTo(HaveOccurred())
		...
		Entry("Empty IPPool", testCaseK8sDeleteAddresses{
			ipPool: &ipamv1.IPPool{},
			ipAddressClaim: &capipamv1.IPAddressClaim{
				ObjectMeta: metav1.ObjectMeta{
					Name: "TestRef",
		...
		Entry("No Deletion needed", testCaseK8sDeleteAddresses{
			ipPool: &ipamv1.IPPool{},
			ipAddressClaim: &capipamv1.IPAddressClaim{
				ObjectMeta: metav1.ObjectMeta{
					Name: "TestRef",
		...
		Entry("Deletion needed, not found", testCaseK8sDeleteAddresses{
			ipPool: &ipamv1.IPPool{
				Status: ipamv1.IPPoolStatus{
					Allocations: map[string]ipamv1.IPAddressStr{
						"TestRef": "192.168.0.1",
		...
		Entry("Deletion needed", testCaseK8sDeleteAddresses{
			ipPool: &ipamv1.IPPool{
				Spec: ipamv1.IPPoolSpec{
					NamePrefix: "abc",
				},
