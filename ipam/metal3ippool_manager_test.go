...
var _ = Describe("Metal3IPPool manager", func() {
	DescribeTable("Test Finalizers",
		func(ipPool *ipamv1.Metal3IPPool) {
		...
		Entry("No finalizers", &ipamv1.Metal3IPPool{}),
		Entry("Additional Finalizers", &ipamv1.Metal3IPPool{
	...
	type testGetIndexes struct {
		ipPool              *ipamv1.Metal3IPPool
		addresses           []*ipamv1.Metal3IPAddress
		expectError         bool
	...
	DescribeTable("Test getIndexes",
		...
		Entry("addresses", testGetIndexes{
			...
			addresses: []*ipamv1.Metal3IPAddress{
				&ipamv1.Metal3IPAddress{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "abc-0",
						Namespace: "myns",
					},
					Spec: ipamv1.Metal3IPAddressSpec{
						Address: "abcd1",
	...
	type testCaseUpdateAddresses struct {
		ipPool                *ipamv1.Metal3IPPool
		ipClaims              []*ipamv1.Metal3IPClaim
		ipAddresses           []*ipamv1.Metal3IPAddress
		expectRequeue         bool
		expectError           bool
	...
	DescribeTable("Test UpdateAddresses",
		func(tc testCaseUpdateAddresses) {
			...
			addressObjects := ipamv1.Metal3IPClaimList{}
	...
	DescribeTable("Test CreateAddresses",
		func(tc testCaseCreateAddresses) {
			...
			// get list of Metal3IPAddress objects
			addressObjects := ipamv1.Metal3IPAddressList{}
			opts := &client.ListOptions{}
			err = c.List(context.TODO(), &addressObjects, opts)
			Expect(err).NotTo(HaveOccurred())
		...
		Entry("Already exists", testCaseCreateAddresses{
			ipPool: &ipamv1.Metal3IPPool{
				ObjectMeta: ipPoolMeta,
				Status: ipamv1.Metal3IPPoolStatus{
	...
	type testCaseDeleteAddresses struct {
		ipPool              *ipamv1.Metal3IPPool
		ipClaim             *ipamv1.Metal3IPClaim
		m3addresses         []*ipamv1.Metal3IPAddress
		addresses           map[ipamv1.IPAddressStr]string
		expectedAddresses   map[ipamv1.IPAddressStr]string
		expectedAllocations map[string]ipamv1.IPAddressStr
		expectError         bool
