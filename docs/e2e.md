...
| `upgrade`  | `clusterctl upgrade` workflow validation from previous release to current provider version                  |
| `rbac`     | OwnerReference setting and operations while ownerReference is used                                          |

### Skip specific test labels

```sh
GINKGO_SKIP_LABELS=features ./hack/e2e/ci-script.sh
```
